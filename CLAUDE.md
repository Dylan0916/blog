...
- **No `src/pages/en/` folder — the `/en` mirror is injected**: there is exactly ONE file per page, under `src/pages/**` (the unprefixed Chinese site). The `i18n-en-mirror` integration (`src/integrations/i18n-en-mirror.ts`, wired in `astro.config.ts`) calls `injectRoute` to mount each root page entrypoint again under an `/en` pattern. Locale resolves from the URL (`Astro.currentLocale`), so the same file renders Chinese at `/` and English at `/en`. **To add a page: create it once under `src/pages/`, then add one line to the `ROUTES` array.** Page bodies live in `src/components/pages/*.astro` (`HomePage`, `TagsPage`, `TagPostsPage`, `SearchPage`, `PostsIndexPage`, `PostPage`, `AboutPage`); the `src/pages/*` files are thin wrappers (and hold `getStaticPaths` for `[slug]`/`[tag]`, shared via `getPostStaticPaths`/`getTagStaticPaths`). `/about` is special: root is `about.md` (auto-routed); `/en/about` injects `AboutPage.astro`, which re-renders the same `about.md` body. Adjacent-post nav carries a `slug` (not a URL) so `AdjacentPostNav` builds the locale-correct link.
  - Why not a single `[lang]` dynamic folder (Nuxt-style)? The default locale is unprefixed (`/`, not `/zh-TW/`), and Astro's rest param `[...lang]` is terminal-only (can't precede `/posts/[slug]`). A normal `[lang]` segment can't be empty for the default. `injectRoute` is the way to keep one source of truth without prefixing the default.
- **Tags** have three forms, all in `src/i18n/tags.ts`: **value** (raw frontmatter string, kept Chinese e.g. `後端` — never edited, so cross-post matching is stable), **slug** (URL segment, always latin via `tagToSlug` — `後端`→`backend`, so URLs are clean: `/en/tags/backend`, not `%E5%BE%8C%E7%AB%AF`), **display** (visible label via `tagDisplay(slug, locale)` — zh shows Chinese, en shows the slug). Latin tags use their slugified value for all three. `tagToSlug` is applied in `getUniqueTags`, `getPostsByTag`, and where `PostLayout` renders `<Tag>`; `Tag.astro` receives a slug and renders `tagDisplay`. To add/adjust a Chinese tag's English slug, edit `TAG_DEFS`.
