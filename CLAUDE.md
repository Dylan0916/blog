...
`zh-TW` default (unprefixed root), `en` mirror under `/en`. `prefixDefaultLocale: false`.

- **Locale source of truth**: `src/i18n/config.ts` (`LOCALES`/`DEFAULT_LOCALE`/`Locale`, dependency-free for `astro.config.ts`). UI strings in `src/i18n/locales/{zh-TW,en}.json` via `useTranslations`; `en.json` is `satisfies`-checked against zh-TW (missing key = compile error).
- **Locale comes from the URL, never a prop**: `.astro` files use `resolveLocale(Astro.currentLocale)`; only React leaves (`Card`, `Datetime`) take a `locale` prop at the render boundary. `counterpartUrl` is derived by `getCounterpartUrl` (`src/utils/getLocale.ts`); `BaseLayout`/`Header` take `langToggle` only to drop the toggle+hreflang on 404.
- **No `src/pages/en/` folder**: one file per page under `src/pages/**`; the `i18n-en-mirror` integration `injectRoute`-mounts each under `/en`. **Add a page → create it once + add one line to `ROUTES`.** Bodies live in `src/components/pages/*.astro`; `[slug]`/`[tag]` share `getStaticPaths` via `get{Post,Tag}StaticPaths`. (Not a single `[lang]` folder because the default is unprefixed and Astro's `[...lang]` is terminal-only.)
- **English post**: `en/<name>.md`, same filename + `postSlug` as the zh twin; paired by base id via `findEnglishTwin` (`src/utils/postLocale.ts`) — no `lang`/`translationKey` frontmatter. Listings stay zh-authored (`getSortedPosts`/`getUniqueTags` filter by `getPostLocale`); on `/en`, `localizedCardData` swaps card title/desc to the twin and `resolveEnglishBody` swaps the detail body.
- **Tags** (`src/i18n/tags.ts`): *value* = raw zh frontmatter (never edited), *slug* = latin via `tagToSlug` (`後端`→`backend`, clean URLs), *display* = `tagDisplay(slug, locale)` (zh Chinese / en slug). Edit `TAG_DEFS` to map a Chinese tag.
- **Out of scope** (don't add unasked): English RSS, per-post English OG image, localized 404, per-locale search index (`/en/search` reuses the one Fuse index).
